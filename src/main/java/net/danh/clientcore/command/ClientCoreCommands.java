...
public final class ClientCoreCommands {
    ...
    public static void register(ClientCore plugin, BlockRegenService blockService, ClientMobService mobService, EditorMenu editorMenu, VisibilityService visibility, LuckService luck, LuckItemService luckItems) {
        plugin.getLifecycleManager().registerEventHandler(LifecycleEvents.COMMANDS, (ReloadableRegistrarEvent<Commands> event) -> {
            LiteralCommandNode<CommandSourceStack> root = Commands.literal("clientcore")
                    ...
                    .then(Commands.literal("editor")
                            ...
                            .executes(context -> {
                                if (!(context.getSource().getSender() instanceof Player player)) {
                                    Text.send(context.getSource().getSender(), "<red>Only players can open the editor.");
                                    return 0;
                                }
                                editorMenu.openMain(player);
                                return 1;
                            }))
                    ...
                    .then(Commands.literal("spawnattr")
                            .requires(source -> source.getSender().hasPermission("clientcore.admin"))
                            .then(Commands.argument("id", StringArgumentType.word())
                                    .suggests((context, builder) -> {
                                        for (String value : mobService.spawnIds()) {
                    ...
                    .then(Commands.literal("debug")
                            ...
                            .then(Commands.argument("key", StringArgumentType.word())
                                    ...
                                    .executes(context -> {
                                        ...
                                        plugin.getConfig().set("settings.debug", enabled);
                                        plugin.saveConfig();
                    ...
                    .build();
            event.registrar().register(plugin.getPluginMeta(), root, "ClientCore admin command", List.of("ccore"));
        });
    }
